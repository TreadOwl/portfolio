...
function Sidebar({
  ...
  if (collapsible === 'none') {
    return (
      <div data-slot="sidebar" className={cn('flex h-full w-(--sidebar-width) flex-col text-sidebar-foreground', className)} {...props}>
        {children}
      </div>
    )
  }
          ...
          style={
            {
              ...
              backgroundImage: 'linear-gradient(to bottom, var(--sidebar) 0%, var(--sidebar-accent) 50%, var(--sidebar-foreground) 100%)',
            } as React.CSSProperties
          }
          side={side}
        >
...
function SidebarSeparator({ className, ...props }: React.ComponentProps<typeof Separator>) {
  return <Separator data-slot="sidebar-separator" data-sidebar="separator" className={cn('w-auto bg-muted-foreground', className)} {...props} />
}
...
function SidebarMenuItem({ className, ...props }: React.ComponentProps<'li'>) {
  return (
    <li
      ...
      className={cn('group/menu-item relative group-data-[collapsible=icon]:justify-items-center', className)}
      {...props}
    />
  )
}
